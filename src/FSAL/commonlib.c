...
void display_fsinfo(struct fsal_module *fsal)
{
	...
	LogDebug(COMPONENT_FSAL, "  no_trunc  = %d ", fsal->fs_info.no_trunc);
	...
	LogDebug(COMPONENT_FSAL, "  umask  = %X ", fsal->fs_info.umask);
	...
	LogDebug(COMPONENT_FSAL, "  pnfs_mds = %d  ", fsal->fs_info.pnfs_mds);
	LogDebug(COMPONENT_FSAL, "  pnfs_ds = %d  ", fsal->fs_info.pnfs_ds);
...
int display_attrlist(struct display_buffer *dspbuf, struct fsal_attrlist *attr,
		     bool is_obj)
{
	...
	if (b_left > 0 && FSAL_TEST_MASK(attr->valid_mask, ATTR_MODE))
		b_left = display_printf(dspbuf, " mode=0%" PRIo32, attr->mode);
		...
		enum fsid_type fsid_type)
{
	...
	case FSID_MAJOR_64:
		memcpy(buf, &fsid->major, sizeof(fsid->major));
		break;

	case FSID_TWO_UINT64:
		memcpy(buf, fsid, sizeof(*fsid));
		break;
	...
	case FSID_DEVICE:
		...
		u32 = fsid->minor;
		memcpy(buf + sizeof(u32), &u32, sizeof(u32));
	}
		...
		enum fsid_type fsid_type)
{
	...
	case FSID_ONE_UINT64:
	case FSID_MAJOR_64:
		memcpy(&fsid->major, buf, sizeof(fsid->major));
		fsid->minor = 0;
		break;

	case FSID_TWO_UINT64:
		memcpy(fsid, buf, sizeof(*fsid));
		break;
	...
	case FSID_DEVICE:
		...
		fsid->major = u32;
		memcpy(&u32, buf + sizeof(u32), sizeof(u32));
		fsid->minor = u32;
		break;
	}
...
static fsal_errors_t dup_ace(fsal_ace_t *sace, fsal_ace_t *dace)
{
	*dace = *sace;
	...
	GET_FSAL_ACE_FLAG(*dace) &=
		~(FSAL_ACE_FLAG_INHERIT | FSAL_ACE_FLAG_NO_PROPAGATE);
...
static fsal_status_t fsal_mode_set_ace(fsal_ace_t *deny, fsal_ace_t *allow,
				       uint32_t mode)
{
	...
	if (mode & S_IWUSR)
		GET_FSAL_ACE_PERM(*allow) |= FSAL_ACE_PERM_WRITE_DATA |
					     FSAL_ACE_PERM_APPEND_DATA;
	else
		GET_FSAL_ACE_PERM(*deny) |= FSAL_ACE_PERM_WRITE_DATA |
					    FSAL_ACE_PERM_APPEND_DATA;
	if (mode & S_IXUSR)
...
static fsal_status_t fsal_mode_gen_set(fsal_ace_t *ace_owner_group,
				       fsal_ace_t *ace_everyone, uint32_t mode)
{
...
static fsal_status_t fsal_mode_gen_acl(struct fsal_attrlist *attrs)
{
...
static bool fsal_check_ace_couple(const fsal_ace_t *aces, uid_t who)
{
...
static bool fsal_can_skip_ace(const fsal_ace_t *ace, uint32_t indx,
			      uint32_t naces, bool can_reuse)
{
...
fsal_status_t fsal_mode_to_acl(struct fsal_attrlist *attrs, fsal_acl_t *sacl)
{
	...
	if ((!can_reuse && naces - acl_data.naces != 6) ||
		...
		LogDebug(COMPONENT_FSAL, "Bad naces: %d not %d", acl_data.naces,
			 naces - 6);
...
static uint32_t ace_modes[3][3] = { { /* owner */
			 ...
			 bool trunc_verif)
{
	...
	LogFullDebug(COMPONENT_FSAL, "Passed verifier %" PRIx32 " %" PRIx32,
		     verf_hi, verf_lo);
	...
	if (isDebug(COMPONENT_FSAL) &&
		...
		LogWarn(COMPONENT_FSAL,
			"atime or mtime was already set in attributes%" PRIx32
			" %" PRIx32,
...
{
	int access_read_inc = ((int)(new_openflags & FSAL_O_READ) != 0) -
			      ((int)(old_openflags & FSAL_O_READ) != 0);

	int access_write_inc = ((int)(new_openflags & FSAL_O_WRITE) != 0) -
			       ((int)(old_openflags & FSAL_O_WRITE) != 0);

	int deny_read_inc = ((int)(new_openflags & FSAL_O_DENY_READ) != 0) -
				   ...
				   fsal_openflags_t openflags, bool bypass)
{
	...
	if ((openflags & FSAL_O_READ) != 0 && share->share_deny_read > 0 &&
	    !bypass) {
...
out_conflict:
	...
	LogDebugAlt(COMPONENT_STATE, COMPONENT_FSAL,
		    "Share conflict detected: %s openflags=%d bypass=%s", cause,
		    (int)openflags, bypass ? "yes" : "no");

	LogFullDebugAlt(
		COMPONENT_STATE, COMPONENT_FSAL,
...
uint32_t lru_try_one(void)
{
	...
	fsal_fd = glist_last_entry(&fsal_fd_global_lru, struct fsal_fd, fd_lru);
...
void fd_lru_run(struct fridgethr_context *ctx)
{
	...
	if (!woke) {
		...
		if (fd_lru_state.futility >= futility_count)
			LogInfo(COMPONENT_FSAL, "Leaving FD futility mode.");
	...
	if (currentopen < fd_lru_state.fds_lowat) {
		...
		atomic_store_uint32_t(&fd_lru_state.fd_state, FD_MIDDLE);
	}
	...
	if (get_open_file_limit(&rlim) != 0) {
		...
		if (rlim.rlim_cur == RLIM_INFINITY) {
			...
			if (code != 1) {
				...
				LogMajor(
					COMPONENT_MDCACHE_LRU,
					"If you are running some other system, please set an rlimit on file descriptors (for example, with ulimit) for this process and consider editing " __FILE__
...
err_open:;
	...
	fd_lru_state.fds_hiwat =
		(params->fd_hwmark_percent * fd_lru_state.fds_system_imposed) /
		100;

	fd_lru_state.fds_lowat =
		(params->fd_lwmark_percent * fd_lru_state.fds_system_imposed) /
		100;
			    ...
			    struct fsal_fd *fsal_fd, bool is_reclaiming)
{
	...
	if (FSAL_IS_ERROR(status)) {
		LogFullDebug(COMPONENT_FSAL, "fsal_start_fd_work returned %s",
			     fsal_err_txt(status));
		return status;
	}
...
fsal_status_t wait_to_start_io(struct fsal_obj_handle *obj_hdl,
			       struct fsal_fd *fsal_fd,
			       fsal_openflags_t openflags, bool may_open,
			       bool may_reopen)
{
	fsal_status_t status = { ERR_FSAL_NO_ERROR, 0 };
	bool retried = false;
...
retry:
	...
	LogFullDebug(COMPONENT_FSAL,
		     "%p try io_work = %" PRIi32 " fd_work = %" PRIi32, fsal_fd,
	...
	while (atomic_fetch_int32_t(&fsal_fd->fd_work) != 0) {
		...
		LogFullDebug(COMPONENT_FSAL,
			     ...
			     (int)fsal_fd->openflags, (int)openflags);
	 ...
	 * open or re-open it.
	 */
	LogFullDebug(COMPONENT_FSAL, "Open mode = %x, desired mode = %x",
		     (int)fsal_fd->openflags, (int)openflags);
	...
	if (!open_correct(fsal_fd->openflags, openflags)) {
		...
		if (retried || cant_reopen(fsal_fd, may_open, may_reopen)) {
			...
			LogFullDebug(
				COMPONENT_FSAL,
				"%p we don't need to claim io_work (-1) = %" PRIi32
				" fd_work = %" PRIi32,
				fsal_fd,
				atomic_fetch_int32_t(&fsal_fd->io_work) - 1,
				atomic_fetch_int32_t(&fsal_fd->fd_work));
				   ...
				   struct fsal_share *share)
{
	...
	if (!open_any && share != NULL) {
		status = check_share_conflict_and_update_locked(
			obj_hdl, share, FSAL_O_CLOSED, openflags, bypass);
...
skip_global_fd:
	...
	if (!open_any && share != NULL) {
		 ...
		 */
		update_share_counters_locked(obj_hdl, share, openflags,
					     FSAL_O_CLOSED);
	}
			    ...
			    bool bypass, struct fsal_share *share)
{
	...
	LogFullDebug(COMPONENT_FSAL, "wait_to_start_io failed returned %s",
		     fsal_err_txt(status));
...
use_related_fd:
		...
		status = wait_to_start_io(obj_hdl, related_fd, openflags, false,
					  false);
		...
		if (FSAL_IS_SUCCESS(status)) {
			 ...
			 */
			LogFullDebug(COMPONENT_FSAL, "Use related_fd %p",
				     related_fd);
			if (out_fd) {
				*out_fd = related_fd;
...
global:
	...
	LogFullDebug(COMPONENT_FSAL, "Use global fd openflags = %x", openflags);
			       ...
			       struct fsal_fd *fsal_fd)
{
	...
	LogFullDebug(COMPONENT_FSAL,
		     ...
		     fsal_fd, atomic_fetch_int32_t(&fsal_fd->io_work) - 1,
		     atomic_fetch_int32_t(&fsal_fd->fd_work));
			 ...
			 bool trunc_verif)
{
	...
	LogFullDebug(COMPONENT_FSAL,
		     ...
		     verf_hi, verf_lo, (uint32_t)st->st_atim.tv_sec,
		     (uint32_t)st->st_mtim.tv_sec);

	return st->st_atim.tv_sec == verf_hi && st->st_mtim.tv_sec == verf_lo;
}
			     ...
			     fsal_verifier_t verifier, bool trunc_verif)
{
	...
	LogFullDebug(COMPONENT_FSAL,
		     "Passed verifier %" PRIx32 " %" PRIx32
		     " file verifier %" PRIx32 " %" PRIx32,
		     verf_hi, verf_lo, (uint32_t)attrs->atime.tv_sec,
		     (uint32_t)attrs->mtime.tv_sec);

	return attrs->atime.tv_sec == verf_hi && attrs->mtime.tv_sec == verf_lo;
}
						  ...
						  struct fsal_pnfs_ds *pds)
{
	...
	if (op_ctx->ctx_export != NULL && op_ctx->ctx_export->fullpath) {
		op_ctx->ctx_fullpath = gsh_refstr_get(
			rcu_dereference(op_ctx->ctx_export->fullpath));
	} else {
	...
	if (op_ctx->ctx_export != NULL &&
		...
		op_ctx->ctx_pseudopath = gsh_refstr_get(
			rcu_dereference(op_ctx->ctx_export->pseudopath));
	} else {
